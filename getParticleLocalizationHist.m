function [ out , loc ] = getParticleLocalizationHist( mapGT , mapSeg , len )
%GETPARTICLELOCALIZATIONHIST Summary of this function goes here
...
comp2=compareMapsHist(mapGT,mapSeg,len);
[indGT,freqGT]=getTopologicalMapHistFix(mapGT);
globalMap.ind=indGT;
...
for I=1:length(globalMap.ind)-2
    ...
    transMat(I,I)=globalMap.len(I)/(globalMap.len(I)+0.0008);%0.9;%globalMap.len(I)/ss;
...
a_P=0.25; % Particle weight memory

for I=2:length(mapSeg)
    
    %Traslacion de particulas
    accum_w=0;
    for J=1:N_part
        particles(J).length= particles(J).length + (mapSeg(I).dist)*(1 + x_N*randn);
